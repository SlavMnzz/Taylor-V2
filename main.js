...
const runTasks = async () => {
  ...
  try {
    const results = await Promise.allSettled(lodash.map(tasks.filter(task => task.run), executeTask));
...
global.reload = async (_ev, filename) => {
  ...
  try {
    if (existsSync(dir)) {
      ...
      const fileContent = readFileSync(dir, "utf-8");
  ...
  try {
    ...
    const deletedFiles = await Promise.all(lodash.map(filenames, async file => {
      try {
        ...
        const stats = statSync(filePath);
        if (stats.isFile() && file.startsWith("pre-key-")) {
          unlinkSync(filePath);
          return filePath;
        }
      } catch (err) {
