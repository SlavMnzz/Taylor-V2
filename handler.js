...
export async function handler(chatUpdate) {
  ...
  try {
    ...
    const isROwner = [this.decodeJid?.(this.user.id), ...lodash.map(owner, ([number]) => number)].map(v => v.replace(/[^0-9]/g, "") + "@s.whatsapp.net").includes(m.sender);
    const isOwner = isROwner;
    const isMods = isOwner || lodash.map(mods, v => v.replace(/[^0-9]/g, "") + "@s.whatsapp.net").includes(m.sender);
    const isPrems = isROwner || lodash.map(prems, v => v.replace(/[^0-9]/g, "") + "@s.whatsapp.net").includes(m.sender);
    ...
    for (let name in plugins) {
      ...
      if (isAcc) {
        ...
        if (plugin.rowner && !(isROwner || isOwner) ? (fail("owner", m, this), true) : plugin.rowner && !isROwner ? (fail("rowner", m, this), true) : plugin.owner && !isOwner ? (fail("owner", m, this), true) : plugin.mods && !isMods ? (fail("mods", m, this), true) : plugin.premium && !isPrems ? (fail("premium", m, this), true) : plugin.group && !m.isGroup ? (fail("group", m, this), true) : plugin.botAdmin && !isBotAdmin ? (fail("botAdmin", m, this), true) : plugin.admin && !isAdmin ? (fail("admin", m, this), true) : plugin.private && m.isGroup ? (fail("private", m, this), true) : plugin.register && !_user.registered ? (fail("unreg", m, this), true) : plugin.nsfw && !db.data?.chats[m.chat]?.nsfw ? (fail("nsfw", m, this), true) : plugins[name]?.error ? (fail("error", m, this), true) : opts["antirpg"] && db.data?.settings[this.user.jid]?.antirpg && plugin.tags?.includes("rpg") ? (fail("rpg", m, this), true) : opts["restrict"] && db.data?.settings[this.user.jid]?.restrict && plugin.tags?.includes("admin") ? (fail("restrict", m, this), true) : false) return false;
        m.isCommand = true;
...
export async function participantsUpdate({
  ...
  switch (action) {
    ...
    case "remove":
      if (chat.welcome) {
        ...
        for (let user of participants) {
          ...
          const welAp = await Welcome(await (conn || this).profilePictureUrl(user, "image").catch(() => hwaifu.getRandom()), await (conn || this).getName(id), groupMetadata.subject?.toString(), parseInt(participants.length), "Welcome to our group!", welcomeText.split(" ").slice(0, 10).join(" "), parseInt(id), thumb);
          ...
          const leaAp = await Leave(await (conn || this).profilePictureUrl(user, "image").catch(() => hwaifu.getRandom()), await (conn || this).getName(id), groupMetadata.subject?.toString(), parseInt(participants.length), "Leave in group!", welcomeText.split(" ").slice(0, 10).join(" "), parseInt(id), thumb);
    ...
    case "promote":
      ...
      const resultPromoteString = lodash.chain(promoteString).split("_").map(word => word === "UNKNOWN" ? "Tidak Diketahui" : lodash.capitalize(lodash.toLower(word))).join(" ").value();
      if (chat.detect) {
    ...
    case "demote":
      ...
      const resultDemoteString = lodash.chain(demoteString).split("_").map(word => word === "UNKNOWN" ? "Tidak Diketahui" : lodash.capitalize(lodash.toLower(word))).join(" ").value();
      if (chat.detect) {
...
export async function groupsUpdate(groupUpdates) {
  for (const groupUpdate of groupUpdates) {
    ...
    const resultString = lodash.chain(inputString).split("_").map(word => word === "UNKNOWN" ? "Tidak Diketahui" : lodash.capitalize(lodash.toLower(word))).join(" ").value();
    if (!text) continue;
  ...
  if (msg) return await conn.reply(m.chat, msg, m, {
    contextInfo: {
      ...
      externalAdReply: {
        ...
        previewType: 0,
        renderLargerThumbnail: false,
        thumbnailUrl: "https://cdn-icons-png.flaticon.com/128/752/752755.png",
        sourceUrl: ""
      }
    }
  });
