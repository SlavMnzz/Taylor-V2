    ...
    sendFile: {
      async value(jid, path, filename = "", caption = "", quoted, ptt = false, options = {}) {
        try {
          ...
          try {
            if (options.asPtv) {
              m = await conn.relayMessage(jid, await generateWAMessageFromContent(jid, proto.Message.fromObject({
                [mtype]: file
              }), {
...
END:VCARD`.trim();
    ...
    sendPayment: {
      async value(jid, amount, currency, text = "", from, image, options) {
        ...
        let a = ["IDR", "RSD", "USD"];
        let b = a[Math.floor(Math.random() * a.length)];
        const messager = await generateWAMessageFromContent(jid, proto.Message.fromObject({
    ...
    sendButtonCta: {
      async value(jid, messages, quoted, options) {
        return messages.length > 1 ? await conn.sendCarousel(jid, messages, quoted, options) : await conn.sendNCarousel(jid, ...messages?.[0], quoted, options);
      }
    },
    sendNCarousel: {
      ...
      async value(jid, messages, quoted, options) {
        ...
        if (messages.length > 1) {
          const cards = await Promise.all(messages.map(async ([text = "", footer = "", buffer = null, buttons = [], copy = null, urls = null, list = null]) => {
            ...
            return {
              ...
              header: proto.Message.InteractiveMessage.Header.fromObject({
                ...
                subtitle: botdate,
                hasMediaAttachment: img?.imageMessage || video?.videoMessage || document?.documentMessage ? true : false,
                imageMessage: img?.imageMessage || null,
                videoMessage: video?.videoMessage || null,
          ...
          const interactiveMessage = proto.Message.InteractiveMessage.create({
            ...
            ...mergeContext(conn, messages[0][0] || defaultText, options)
          });
          const messageContent = proto.Message.fromObject({
            viewOnceMessage: {
              message: {
          ...
          return await conn.sendNCarousel(jid, ...messages?.[0], quoted, options);
        }
      }
    },
    sendCta: {
    ...
    reply: {
      async value(jid, text = "", quoted, options = {}) {
        try {
          ...
          let m = null;
          try {
            m = data ? await conn.sendFile(jid, data, "file", "", quoted, false, options) : await conn.sendMessage(jid, {
              text: text,
              ...mergeContext(conn, text, options)
            }, {
              quoted: quoted,
    ...
    sendHydrated: {
      async value(jid, text = "", footer = "", b, c = "", d = "", e = "", f = "", buttons, quoted, options) {
        ...
        return !b ? await conn.sendMessage(jid, {
          ...
          quoted: quoted,
          ...options
        }) : await conn.sendFile(jid, b, footer, text + "\n\n" + readMore + "\n\n" + footer + "\n\n" + c + "\n\n" + d + "\n\n" + e + "\n\n" + f + "\n\n" + button.map(([title, command]) => ({
          title: title,
          command: command
        })).map((v, index) => {
          return `*[ ${index + 1} ] Name:* ${v.title || ""}\n➥ *CMD:* ${v.command || ""}`.trim();
    ...
    loadMessage: {
      value(jid = conn.user.jid, id = null, option = 3) {
        ...
        return option === 1 ? (typeof store?.messages?.[jid]?.get === "function" ? store.messages[jid].get(id) : _.find(_.flatten(_.map(store?.messages?.[jid]?.values() || [], "array")), ({
          key
        }) => key.id === id)) || null : option === 2 ? _.find(_.get(conn.chats, jid, {}).messages || [], ({
          key
        }) => key.id === id) || null : (typeof store?.messages?.[jid]?.get === "function" ? store.messages[jid].get(id) : _.find(_.flatten(_.map(store?.messages?.[jid]?.values() || [], "array")), ({
          key
        }) => key.id === id)) || _.find(_.get(conn.chats, jid, {}).messages || [], ({
          key
        }) => key.id === id) || store?.loadMessage(jid, id) || {};
      },
      enumerable: true
    },
    sendGroupV4Invite: {
  ...
  return Object.defineProperties(proto.WebMessageInfo.prototype, {
    ...
    quoted: {
      get() {
        ...
        const quoted = contextInfo?.quotedMessage || _contextInfo?.quotedMessage;
        ...
        return Object.defineProperties(JSON.parse(JSON.stringify(typeof q === "string" ? {
          ...
          upload: {
            async value(saveToFile = false) {
              const mtype = this.mediaType;
              return await uploadFile(await self.conn?.downloadM(this.mediaMessage[mtype], mtype.replace(/message/i, ""), saveToFile));
            },
            enumerable: true,
            configurable: true
          },
    ...
    upload: {
      async value(saveToFile = false) {
        const mtype = this.mediaType;
        return await uploadFile(await this.conn?.downloadM(this.mediaMessage[mtype], mtype.replace(/message/i, ""), saveToFile));
      },
      enumerable: true,
      configurable: true
    },
    ...
    getQuotedObj: {
      value() {
        ...
        const message = proto.WebMessageInfo.fromObject(this.conn?.loadMessage(this.chat, this.quoted.id) || this.conn?.loadMessage(null, this.quoted.id) || store?.loadMessage(this.chat, this.quoted.id) || this.quoted.vM);
